...
class LoginPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => sl<SsoLoginCubit>()),
      ],
    ...
    return Scaffold(
      body: MultiBlocListener(
        listeners: [
          ...
          BlocListener<SsoLoginCubit, SsoLoginState>(
            listener: (context, state) {
              if (state is SsoLoginSuccess) {
                Navigator.of(context).pushReplacement(
                  MaterialPageRoute(builder: (_) => const HomeScreen()),
                );
              } else if (state is SsoLoginUserNotFound) {
                _showErrorDialog(context, "Account not found. Please sign up first.");
              } else if (state is SsoLoginError) {
                _showErrorDialog(context, state.message);
              }
            },
          ),
        ...
        child: BlocBuilder<AuthCubit, AuthState>(
          builder: (context, authState) {
            final ssoState = context.watch<SsoLoginCubit>().state;
            final isLoading =
                authState is AuthLoading || ssoState is SsoLoginLoading;
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: () {
                              if (formKey.currentState!.validate()) {
                                context.read<SsoLoginCubit>().loginWithSso(
                                      email: emailController.text.trim(),
                                      password: passwordController.text.trim(),
                                      productKey: "CARDINAL",
                                    );
